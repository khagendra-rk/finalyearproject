...
<div class="card">
    <div class="card-header">
        <h3 class="card-title" style="font-size: 1.5em;font-weight:bold">Create New Users</h3>
...
<div class="card-body">
    ...
    <div class="form-group">
        ...
        <select name="role" id="role" class="form-control @error('role') is-invalid @endif">
            <option value="0"@if(old('data') && old('data')==0) selected @endif>User</option>
            <option value="1"@if(old('data') && old('data')==0) selected @endif>Employee</option>
            <option value="2"@if(old('data') && old('data')==0) selected @endif>Admin</option>
        @error('role')
        <small class="form-text text-danger">{{ $message }}</small>       
        @enderror
    </div>
